                ...
                children: [
                  ...
                  _buildArticle(
                    ...
                    items: [
                      ...
                      '"회원"이란 본 약관에 동의하고 회사와 이용계약을 체결한 자로, \'게스트\'와 \'호스트\'로 구분됩니다.',
                      '"게스트"란 공간 및 옵션 상품을 예약하고 결제하여 이용하는 회원을 말합니다.',
                      '"호스트"란 자신의 공간 정보를 등록하고 게스트에게 공간을 제공하는 회원을 말합니다.',
                      '"옵션 상품"이란 게스트가 단기 거주 편의를 위해 공간과 함께 선택하여 구매 또는 대여하는 침구류, 생활 비품 등을 말합니다.',
                  ...
                  _buildArticle(
                    ...
                    subItems: {
                      0: [
                        ...
                        '욕설, 협박 등으로 호스트, 게스트 또는 회사 임직원의 업무를 방해하거나 명예를 훼손하는 행위',
                      ],
                    },
                  ),
                  _buildArticle(
                  ...
                  _buildArticle(
                    ...
                    null,
                    items: [
                      '계약은 게스트가 임대 공간 및 옵션 상품을 선택하고 결제를 완료함과 동시에 성립됩니다.',
                  ...
                  _buildArticle(
                    ...
                    null,
                    items: [
                      '보증금은 게스트 퇴실 후 호스트가 \'퇴실 확인\'을 완료하면 즉시 반환됩니다.',
                      '회사가 운영정책으로 정한 기간 내에 호스트의 퇴실 확인 처리가 없을 경우, 시스템에 의해 자동으로 퇴실이 확인되며 보증금은 전액 반환됩니다.',
                      '시설 훼손 등으로 호스트가 \'퇴실 확인 보류\'를 신청할 경우, 회사는 별도의 운영정책에서 정한 최대 보관 기간 동안만 보증금을 예치합니다. 해당 기간이 경과할 때까지 양측 합의가 성립되지 않으면, 회사는 보증금을 게스트에게 우선 자동 반환하며, 이후의 손해배상은 당사자 간 직접 해결하여야 합니다.',
                    ],
                  ),
                  _buildArticle(
                    '제12조 (호스트 정산)',
                    null,
                    items: [
                      '회사는 게스트가 결제한 총 임대료에서 회사의 서비스 수수료를 차감한 금액을 호스트에게 정산합니다.',
                      '정산금은 게스트의 정상적인 입주 후, 회사의 운영정책에서 별도로 정한 정산 주기에 따라 호스트의 계좌로 지급됩니다.',
                      '계약 취소 또는 호스트의 귀책사유로 정상적인 거주가 불가능했던 경우, 회사는 정산금 지급을 보류하거나 기지급된 금액의 환수를 요청할 수 있습니다.',
                    ],
                  ),
                  _buildArticle(
                    ...
                    null,
                    items: [
                      '회사는 통신판매중개자로서 회원 간의 편리한 거래를 위한 플랫폼을 제공할 뿐, 임대차 계약의 직접 당사자가 아닙니다. 따라서 공간의 상태, 적법성, 게스트의 신원 등에 대해 회사는 보증하지 않으며 법적 책임을 지지 않습니다.',
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSubItem(textStyle, '① 계약 취소 시 환불 기준은 호스트가 해당 공간에 당시 설정한 \'개별 환불 정책\'을 최우선으로 적용합니다.'),
          _buildSubItem(textStyle, '② 게스트는 원칙적으로 입주일 이후에는 임의로 계약을 취소할 수 없습니다. 부득이한 사유로 입주 후 중도 퇴실 및 취소를 원할 경우에는 호스트와 별도로 계약 해지 및 환불 조건을 합의하여 환불 신청을 할 수 있습니다.'),
          _buildSubItem(textStyle, '③ 회사는 공간 임대료 환불 시, 호스트의 환불 규정에 따른 \'취소 위약금\' 및 회사의 \'서비스 수수료\'를 차감한 잔액을 게스트에게 환불 처리합니다. (단, 결제 수단에 따라 실제 환불 완료까지 영업일 기준 일정 시일이 추가 소요될 수 있습니다.)'),

          // 2. 호스트에 의한 계약 취소
          Padding(
            padding: const EdgeInsets.only(top: 12),
            child: Text('2. 호스트에 의한 계약 취소 (페널티)', style: subTitleStyle),
          ),
          _buildSubItem(textStyle, '① 호스트의 개인적 사정, 이중 예약 등 부득이한 귀책사유로 계약 취소를 원할 경우, 호스트는 회사에 이를 즉시 통보하여야 합니다. 이 경우 호스트는 회사의 운영정책에 따라 호스트가 설정한 환불 정책에 따라 위약금을 지불할 책임이 있으며, 회사는 호스트에게 서비스 이용 제한 등의 페널티를 부과할 수 있습니다.'),
